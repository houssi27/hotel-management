...
AppsMenu.include({
   init: function (parent, menuData) {
        ...
        this._hide_icons = session.get_cookie('hide_icons');
        },
   start: function () {
    	var res = this._super.apply(this, arguments);
        this.$el.find("[data-hide-icons='1']").removeClass('show hide-top');
        return res;
    },
   _onAppsMenuItemClicked: function (ev) {
     // add the class to make sure we don't remove the menu until the view is loaded
     $(ev.target).closest('.dropdown').addClass('show');
     this._super.apply(this, arguments);
     session.set_cookie('hide_icons', true);
     setTimeout(function(){
        $(ev.target).closest('.dropdown-menu').removeClass('hide-top');
     }, 3000)
